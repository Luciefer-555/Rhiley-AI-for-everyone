...
import ScrapedPreviewCard from "./ScrapedPreviewCard"
                  ...
                  <Message role={m.role}>
                    {m.role === "user" ? (
                      ...
                      <>
                        ...
                        {m.type === "scraped-preview" && m.scrapeData && (
                          <div className="mt-2">
                            <ScrapedPreviewCard
                              {...m.scrapeData}
                              onUseCode={(code) => {
                                // Simulate user pasting code back in to use
                                fetch('/api/build-status', {
                                  method: 'POST',
                                  headers: { 'Content-Type': 'application/json' },
                                  body: JSON.stringify({ code: code, status: 'READY' })
                                }).catch(e => console.error("Bridge update failed:", e));
                                onSend?.("Applied the scraped component to the preview.");
                              }}
                              onRefine={(refinementPrompt) => onSend?.(refinementPrompt)}
                            />
                          </div>
                        )}
                        {(m.type === "text" || !m.type) && (
