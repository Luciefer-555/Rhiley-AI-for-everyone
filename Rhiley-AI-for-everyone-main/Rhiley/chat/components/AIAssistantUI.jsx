...
import { detectBrowserIntent, formatScrapeStartMessage, formatScrapeSuccessMessage } from "../lib/browserToolDetector"
...
const AIAssistantUI = memo(function AIAssistantUI({
  ...
  async function sendMessage(convId, contentPayload, aestheticOverride = null) {
    ...
    // ── INTERCEPT: browser intelligence (scraping) ─────────
    const browserIntent = detectBrowserIntent(textContent);
    if (browserIntent.isBrowserRequest && browserIntent.url) {
      setIsThinking(true);
      setThinkingConvId(convId);
      ...
      const loadingMsg = {
        id: loadingId,
        role: "assistant",
        content: formatScrapeStartMessage(browserIntent),
        createdAt: new Date().toISOString(),
      };
      ...
      try {
        const res = await fetch('/api/scrape', {
          ...
          body: JSON.stringify({
            ...
            userRequest: browserIntent.userRequest,
        ...
        if (data.success && data.adaptedCode) {
          // Send code to live preview
          fetch('/api/build-status', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ code: data.adaptedCode, status: 'READY' })
          }).catch(e => console.error("Bridge update failed:", e));

          const asstMsg = {
            id: Math.random().toString(36).slice(2),
            role: "assistant",
            content: formatScrapeSuccessMessage(data.pageTitle, data.libraries, data.profile),
            type: "scraped-preview",
            scrapeData: {
              adaptedCode: data.adaptedCode,
          ...
          if (onSaveMessage) onSaveMessage("assistant", asstMsg.content);
          ...
          const errorMsg = {
            id: Math.random().toString(36).slice(2),
            role: "assistant",
            content: `⚠️ Scraping failed: ${data.error || "Unknown error"}`,
            createdAt: new Date().toISOString(),
            isNew: true,
          };
          setConversations((prev) => prev.map((c) => c.id === convId ? { ...c, messages: [...(c.messages || []), errorMsg] } : c));
        ...
        const errorMsg = {
          id: Math.random().toString(36).slice(2),
          role: "assistant",
          content: `⚠️ Scraping failed: Could not reach the server.`,
          createdAt: new Date().toISOString(),
          isNew: true,
        };
        setConversations((prev) => prev.map((c) => c.id === convId ? { ...c, messages: [...(c.messages || []), errorMsg] } : c));
